import { OrganizationFile, OrganizationInternationalization, OrganizationSettingsDTO } from '../types';

export const organizationSettingsSerializer = (data: any): OrganizationSettingsDTO => ({
    organizationId: data.settings.organizationId,
    ownerReference: data.settings.ownerReference !== '-' ? data.settings.ownerReference : '',
    organizationName: data.settings.organizationName !== '-' ? data.settings.organizationName : '',
    ...
    internationalization: data.settings.internationalization.map(
        (internationalization: any) =>
            ...
            } as OrganizationInternationalization)
    ),
    files: data.settings.files.map(
        (file: any) =>
            ({
